...
# I could have broken smth here
  ...
  #       'hosts not found': 'hosts_not_found/'
    ...
    #           lambda row: f"<a class='btn btn-secondary' role='button' href='{request.host_url}anomalous_paths/site={row['site']}&date={row['to']}&id={row['alarm_id']}' target='_blank'>VIEW IN A NEW TAB</a>" if row['site'] and row['to'] else '-', axis=1)
    #   elif 'alarm_link' in df.columns:
